...
public class newMemberGUI extends JFrame
{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			if (e.getSource() == signUp)
			{
				...
				} else
				{
					...
					{
						if (fileAdapter.getMembersList().getMembersList().get(i).getMemberID() == Integer
								.parseInt(memberIDInput.getText()))
						{
					...
					for (int i = 0; i < fileAdapter.getEventsList().getEventsList().size(); i++)
					{
						if (fileAdapter.getEventsList().getEventsList().get(i).toSmallString()
								.equals(allEvents.getSelectedValue()))
						{
					...
					for (int i = 0; i < fileAdapter.getEventsList().getEventsList().size(); i++)
					{
						...
						if (fileAdapter.getEventsList().getEventsList().get(i).getClassTypeString()
								.equals(classTypeInput.getSelectedItem()))
						{
							listModel.addElement(fileAdapter.getEventsList().getEventsList().get(i).toSmallString());
						}
					}
			...
			if (e.getSource() == save)
			{
				...
				if (alreadyMember == true)
				{
					...
					tempMember.setMemberSince(new MyDate(Integer.parseInt(membershipSinceInputDay.getText()),
							Integer.parseInt(membershipSinceInputMonth.getText()),
							Integer.parseInt(membershipSinceInputYear.getText())));
				...
				} else
				{
					...
					MyDate memberSince = new MyDate(Integer.parseInt(membershipSinceInputDay.getText()),
							Integer.parseInt(membershipSinceInputMonth.getText()),
							Integer.parseInt(membershipSinceInputYear.getText()));
					String email = emailInput.getText();
					String phone = phoneInput.getText();
					int newMemberId = Integer.parseInt(memberIDInput.getText() + "");
					tempMember = new Member(name, email, phone, newMemberId, type);
			...
			{
				fileAdapter.updateMembersList();
				int yesno = JOptionPane.showConfirmDialog(null, "Are you sure you want to delete this member?",
						"Confirm before deleting member", JOptionPane.YES_NO_OPTION);
			...
			if (e.getSource() == classTypeInput)
			{
				...
				for (int i = 0; i < fileAdapter.getEventsList().getEventsList().size(); i++)
				{
					...
					if (fileAdapter.getEventsList().getEventsList().get(i).getClassTypeString()
							.equals(classTypeInput.getSelectedItem().toString()))
					{
						listModel.addElement(fileAdapter.getEventsList().getEventsList().get(i).toSmallString());
					}
				}
		...
		public void focusGained(FocusEvent e)
		{
			if (e.getSource() == membershipSinceInputDay)
			{
				if (membershipSinceInputDay.getText().equals("Day"))
				{
					...
					membershipSinceInputDay.setText(membershipSinceInputDay.getText());
			}
			if (e.getSource() == membershipSinceInputMonth)
			{
				if (membershipSinceInputMonth.getText().equals("Month"))
				{
					...
					membershipSinceInputMonth.setText(membershipSinceInputMonth.getText());
			}
			if (e.getSource() == membershipSinceInputYear)
			{
				if (membershipSinceInputYear.getText().equals("Year"))
				{
					...
					membershipSinceInputYear.setText(membershipSinceInputYear.getText());
			}
		}
	...
	public void fillWithMember(Member member)
	{
		...
		if (member.getIsPremium())
		{
			...
			membershipTypeInput.setSelectedIndex(1);
