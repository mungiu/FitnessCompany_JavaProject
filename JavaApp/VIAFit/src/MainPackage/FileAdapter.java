...
{
   String membersListBinFileName, instructorsListBinFileName,
         eventsListBinFileName, classTypeListBinFileName;
   ...
   public void updateOnGoingEventsList()
   {
      ...
      MyClock currentTime = new MyClock(
            calendar.get(GregorianCalendar.HOUR_OF_DAY), 0, 0);
...

      {
         ...
         boolean eventStarted = thisEventStartTime.getHour() <= currentTime
               .getHour();
         boolean eventDidNotFinish = thisEventEndTime.getHour() > currentTime
               .getHour();
         boolean eventFinished = thisEventEndTime.getHour() < currentTime
               .getHour();
         boolean containsEvent = onGoingEventsList.contains(temp.get(i));

         if (eventIsToday && eventStarted && eventDidNotFinish
               && !containsEvent)
         {
   ...
   /**
    * Updates the upComingEventsList.
    */
   public void updateUpComingEventsList()
   {
      ...
      for (int i = 0; i < temp.size(); i++)
      {
         ...
         boolean eventIsUpcomingYears = thisEventStartDate.getYear() > today
               .getYear();
         boolean eventIsUpcomingMonths = thisEventStartDate.getMonth() > today
               .getMonth();
         boolean eventIsUpcomingDays = thisEventStartDate.getDay() > today
               .getDay();
         boolean eventIsUpcomingHours = thisEventStartTime
         ...
         boolean eventWasBeforeThisYear = thisEventEndDate.getYear() < today
               .getYear();
         boolean eventWasBeforeThisMonth = thisEventEndDate.getMonth() < today
               .getMonth();
         boolean eventWasBeforeThisDay = thisEventEndDate.getDay() < today
               .getDay();
         boolean eventWasBeforeThisHour = thisEventEndTime
               .getHour() < currentTime.getHour();
